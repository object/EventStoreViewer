...
let showSearchModes searchMode dispatch =
  let programColor, intervalColor = 
    ...
    | ByIdOrProgram -> (Color.IsPrimary, Color.IsLight)
    | ByInterval -> (Color.IsLight, Color.IsPrimary)
  Field.div [Field.IsGrouped] [
      showButton "Interval" intervalColor false SearchModeByInterval dispatch
      showSpace ()
      showButton "Id/Program" programColor false SearchModeByIdOrProgram dispatch ]
...
let showIdOrProgramInput model dispatch =
  let inputColor = if Model.hasValidIdOrProgram model then Color.IsSuccess else Color.IsDanger
  let textColor = if model.SearchMode = ByIdOrProgram then Color.IsBlack else Color.IsLight
  Field.div [] [
    Control.div [] [
      Input.text
        [ Input.Color inputColor
          Input.Modifiers [Modifier.TextColor textColor]
          Input.Disabled (model.SearchMode <> ByIdOrProgram)
          Input.OnChange (fun evt -> dispatch (ProgramIdChanged evt.Value))
          Input.DefaultValue model.IdOrProgram
          Input.Placeholder "Id/Program" ] ] ]
